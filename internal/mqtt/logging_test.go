...
func TestObserverLogsTrafficAtDebugAndBrokerCompletionAtTrace(t *testing.T) {
	...
	o.event(bus.FrameHeard{Relay: "mc", Correlation: id, At: time.Now(), Raw: raw})
	...
	for _, entry := range observed.All() {
		if got := entry.ContextMap()["corr"]; got != id.Short() {
			t.Errorf("%q corr = %v, want %s", entry.Message, got, id.Short())
		}
	}
}
...
func TestFailedPublicationDoesNotClaimBrokerCompletion(t *testing.T) {
	...
	if got := failed.ContextMap()["corr"]; got != id.Short() {
		t.Errorf("publish failure corr = %v, want %s", got, id.Short())
	}
}
...
func TestDebugLevelHidesBrokerCompletionDetail(t *testing.T) {
	...
	if got := selected.ContextMap()["corr"]; got != id.Short() {
		t.Errorf("selected frame corr = %v, want %s", got, id.Short())
	}
}

func TestObserverFrameDecisionsNeverLoseTheirCorrelation(t *testing.T) {
	raw, _ := advertFrame(t)
	tests := []struct {
		name  string
		cfg   Config
		event func(correlation.ID) bus.Event
	}{
		{
			name: "rx disabled",
			cfg:  Config{Relay: "mc"},
			event: func(id correlation.ID) bus.Event {
				return bus.FrameHeard{Relay: "mc", Correlation: id, Raw: raw}
			},
		},
		{
			name: "empty rx",
			...
			event: func(id correlation.ID) bus.Event {
				return bus.FrameHeard{Relay: "mc", Correlation: id}
			},
		},
		{
			name: "invalid rx",
			...
			event: func(id correlation.ID) bus.Event {
				return bus.FrameHeard{Relay: "mc", Correlation: id, Raw: []byte{1}}
			},
		},
		{
			name: "type filtered",
		...
		},
		{
			...
			event: func(id correlation.ID) bus.Event {
				return bus.FrameSent{Relay: "mc", Correlation: id, Raw: raw, Shadow: true}
			},
		},
		{
			name: "tx disabled",
			...
			event: func(id correlation.ID) bus.Event {
				return bus.FrameSent{Relay: "mc", Correlation: id, Raw: raw}
			},
		},
	}
