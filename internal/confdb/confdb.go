...
// assignSingleton decodes one singleton block into its pointer.
func assignSingleton(f *config.File, kind string, attrs []byte) error {
	switch kind {
	case KindSentinel:
	...
	case KindSystem:
		...
		f.System = &sys
	case KindUpdate:
		u, err := fromAttrs[config.Update](attrs)
		if err != nil {
			return err
		}
		f.Update = &u
