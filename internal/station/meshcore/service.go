...
func resolve(cfg map[string]any) (params, *mesh.LocalIdentity, error) {
	...
	if p.FrequencyHz == 0 {
		return p, nil, errors.New("meshcore station params: frequency_hz is required")
	}
	if p.FrequencyHz%1_000 != 0 {
		return p, nil, fmt.Errorf("meshcore station params: frequency_hz %d cannot be represented by the companion protocol's kHz field",
			p.FrequencyHz)
	}
	if p.SpreadingFactor < 5 || p.SpreadingFactor > 12 {
		return p, nil, fmt.Errorf("meshcore station params: spreading_factor %d — want 5..12", p.SpreadingFactor)
	}
	if p.BandwidthHz < 7_000 || p.BandwidthHz > 500_000 {
		return p, nil, fmt.Errorf("meshcore station params: bandwidth_hz %d — want 7000..500000", p.BandwidthHz)
	}
	if p.CodingRate < 5 || p.CodingRate > 8 {
		return p, nil, fmt.Errorf("meshcore station params: coding_rate %d — want 5..8", p.CodingRate)
	}
	if len(p.NodeName) > maxStationName {
		return p, nil, fmt.Errorf("meshcore station params: node_name is %d bytes — at most %d fit",
			len(p.NodeName), maxStationName)
	}
	if p.PIN > maxConnectionPIN {
		return p, nil, fmt.Errorf("meshcore station params: pin %d does not fit uint32", p.PIN)
	}
	if p.NodeLat < -90 || p.NodeLat > 90 || p.NodeLon < -180 || p.NodeLon > 180 {
		return p, nil, fmt.Errorf("meshcore station params: invalid location %g,%g", p.NodeLat, p.NodeLon)
	}
	if p.MultiACKs < 0 || p.MultiACKs > 3 {
		return p, nil, fmt.Errorf("meshcore station params: multi_acks %d — want 0..3", p.MultiACKs)
	}
	if p.AdvertLoc < 0 || p.AdvertLoc > math.MaxUint8 || p.TelemetryMode < 0 || p.TelemetryMode > math.MaxUint8 {
		return p, nil, errors.New("meshcore station params: advert_loc_policy and telemetry_mode must fit one byte")
	}
	if p.PathHashMode < 0 || p.PathHashMode > 2 {
		return p, nil, fmt.Errorf("meshcore station params: path_hash_mode %d — want 0..2", p.PathHashMode)
	}
	if p.DutyCyclePct <= 0 || math.IsNaN(p.DutyCyclePct) || math.IsInf(p.DutyCyclePct, 0) || p.DutyCyclePct > 100 {
		return p, nil, fmt.Errorf("meshcore station params: duty_cycle_pct %g — want a finite value in (0,100]",
			p.DutyCyclePct)
	}
	if p.MaxContacts == 0 {
	...
	if p.MaxContacts < 2 || p.MaxContacts > maxContactSlots || p.MaxContacts%2 != 0 {
		return p, nil, fmt.Errorf("meshcore station params: max_contacts %d — want an even value in 2..%d",
			p.MaxContacts, maxContactSlots)
	}
	if p.MaxChannels < 1 || p.MaxChannels > maxChannelSlots {
		return p, nil, fmt.Errorf("meshcore station params: max_channels %d — want 1..%d",
			p.MaxChannels, maxChannelSlots)
	}
	if p.MailboxCap < 1 || p.MailboxCap > maxMailboxSlots {
		return p, nil, fmt.Errorf("meshcore station params: mailbox_capacity %d — want 1..%d",
...
func (s *service) serveClient(ctx context.Context, conn net.Conn, generation uint64) {
	...
	for ctx.Err() == nil {
		...
		if err != nil {
			...
			responses = []companion.Response{companion.ErrorResponse{Code: code}}
		} else {
...
func (s *service) handle(cmd companion.Command) []companion.Response {
	s.mu.Lock()
	defer s.mu.Unlock()
	ok := func() []companion.Response {
		return []companion.Response{companion.StatusResponse(companion.ResponseOK)}
	}
	errResponse := func(code companion.ErrorCode) []companion.Response {
	...
	case companion.AppStart:
		return []companion.Response{s.selfInfo()}
	case companion.GetContacts:
		return []companion.Response{companion.ContactsStart{}, companion.EndOfContacts{}}
	case companion.SetDeviceTime:
		current := time.Now().Add(s.clockDelta)
		requested := time.Unix(int64(c.UnixSeconds), 0)
		if requested.Before(current.Truncate(time.Second)) {
			return errResponse(companion.ErrorIllegalArgument)
		}
	...
	case companion.SetRadioParams:
		...
		if c.FrequencyKHz < 150_000 || c.FrequencyKHz > 2_500_000 ||
	...
	case companion.SetRadioTXPower:
		// Lotor's first station radio is the SX1262, whose reference
		// companion range is -9..22 dBm. The physical envelope is judged
		// again when a send is admitted to an attached radio.
		if c.PowerDBm < -9 || c.PowerDBm > 22 {
			return errResponse(companion.ErrorIllegalArgument)
		}
		s.p.TXPowerDBm = c.PowerDBm
		return ok()
	case companion.GetChannel:
		ch, exists := s.channels[c.Index]
		if !exists || int(c.Index) >= s.p.MaxChannels {
			return errResponse(companion.ErrorNotFound)
		}
		return []companion.Response{companion.ChannelInfo{Index: c.Index, Name: ch.name, Secret: ch.secret}}
	...
	case companion.SimpleCommand:
		...
		default:
			return errResponse(companion.ErrorUnsupportedCommand)
		}
